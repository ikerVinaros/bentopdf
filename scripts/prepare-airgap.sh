...
# ============================================================
# Vinaros Air-Gapped Deployment Preparation Script
# ============================================================
...
# contains everything needed to deploy Vinaros offline.
#
# Usage:
...
WASM_BASE_URL=""
IMAGE_NAME="vinaros"
OUTPUT_DIR="./vinaros-airgap-bundle"
SIMPLE_MODE=""
BASE_URL=""
COMPRESSION_MODE=""
LANGUAGE=""
...
usage() {
  cat <<'EOF'
Vinaros Air-Gapped Deployment Preparation
...
OPTIONS:
  --image-name <name>     Docker image name          (default: vinaros)
  --output-dir <path>     Output bundle directory     (default: ./vinaros-airgap-bundle)
...
if [ ! -f "package.json" ] || [ ! -f "src/js/const/cdn-version.ts" ]; then
  error "This script must be run from the Vinaros project root."
...
interactive_mode() {
  echo ""
  echo -e "${BOLD}============================================================${NC}"
  echo -e "${BOLD}  Vinaros Air-Gapped Deployment Preparation${NC}"
  ...
  echo "      https://cdn.mycompany.local/vinaros"
  echo ""
  while true; do
  ...
  echo "    Replace the default Vinaros name, logo, and footer text."
  read -r -p "    Brand name [Vinaros]: " input
  BRAND_NAME="${input:-}"
  if [ -n "$BRAND_NAME" ]; then
    ...
    BRAND_LOGO="${input:-}"
    read -r -p "    Footer text [© 2026 Vinaros. All rights reserved.]: " input
    FOOTER_TEXT="${input:-}"
  fi
  echo ""
  ...
  echo "  Brand Name:     ${BRAND_NAME:-Vinaros (default)}"
...
echo ""
echo -e "${BOLD}============================================================${NC}"
echo -e "${BOLD}  Vinaros Air-Gapped Bundle Preparation${NC}"
...
if ls "$OUTPUT_DIR"/*.tgz "$OUTPUT_DIR"/vinaros.tar "$OUTPUT_DIR"/setup.sh 2>/dev/null | head -1 &>/dev/null; then
...
if [ "$SKIP_WASM" = true ]; then
  ...
  wasm_missing=false
  if ! ls "$OUTPUT_DIR"/vinaros-pymupdf-wasm-*.tgz &>/dev/null; then
    error "Missing: vinaros-pymupdf-wasm-*.tgz"
    wasm_missing=true
  fi
  if ! ls "$OUTPUT_DIR"/vinaros-gs-wasm-*.tgz &>/dev/null; then
    error "Missing: vinaros-gs-wasm-*.tgz"
    wasm_missing=true
  fi
  ...
  info "Downloading @vinaros/pymupdf-wasm@${PYMUPDF_VERSION}..."
  if ! (cd "$WASM_TMP" && npm pack "@vinaros/pymupdf-wasm@${PYMUPDF_VERSION}" --quiet 2>&1); then
    error "Failed to download @vinaros/pymupdf-wasm@${PYMUPDF_VERSION}"
    error "Check your internet connection and that the package exists on npm."
    exit 1
  fi

  info "Downloading @vinaros/gs-wasm@${GS_VERSION}..."
  if ! (cd "$WASM_TMP" && npm pack "@vinaros/gs-wasm@${GS_VERSION}" --quiet 2>&1); then
    error "Failed to download @vinaros/gs-wasm@${GS_VERSION}"
    error "Check your internet connection and that the package exists on npm."
    exit 1
  fi
  ...
  info "  PyMuPDF:      $(filesize "$OUTPUT_DIR"/vinaros-pymupdf-wasm-*.tgz)"
  info "  Ghostscript:  $(filesize "$OUTPUT_DIR"/vinaros-gs-wasm-*.tgz)"
...
if [ "$SKIP_DOCKER" = true ]; then
  ...
  if [ -f "$OUTPUT_DIR/vinaros.tar" ]; then
    success "Reusing existing vinaros.tar"
  elif docker image inspect "$IMAGE_NAME" &>/dev/null; then
    step "Exporting existing Docker image"
    docker save "$IMAGE_NAME" -o "$OUTPUT_DIR/vinaros.tar"
    success "Exported: $(filesize "$OUTPUT_DIR/vinaros.tar")"
  else
    warn "No Docker image '${IMAGE_NAME}' found and no vinaros.tar in output."
...
# ============================================================
# Vinaros Air-Gapped Setup Script
# Generated on $(date -u +"%Y-%m-%d %H:%M:%S UTC")
# Vinaros v${APP_VERSION}
# ============================================================
...
if [ -f "\${SCRIPT_DIR}/vinaros.tar" ]; then
  docker load -i "\${SCRIPT_DIR}/vinaros.tar"
  echo "  Docker image '\${IMAGE_NAME}' loaded."
else
  echo "  WARNING: vinaros.tar not found. Skipping Docker load."
...
tar xzf "\${SCRIPT_DIR}"/vinaros-pymupdf-wasm-*.tgz -C "\${WASM_DIR}/pymupdf" --strip-components=1

# Ghostscript: browser expects gs.js and gs.wasm at root
echo "  Extracting Ghostscript..."
TEMP_GS="\$(mktemp -d)"
tar xzf "\${SCRIPT_DIR}"/vinaros-gs-wasm-*.tgz -C "\${TEMP_GS}"
...
# --- Step 3: Start Vinaros ---
echo ""
echo "[3/3] Ready to start Vinaros"
echo ""
echo "  To start manually:"
echo "    docker run -d --name vinaros -p \${DOCKER_PORT}:8080 --restart unless-stopped \${IMAGE_NAME}"
echo ""
echo "  Then open: http://localhost:\${DOCKER_PORT}"
echo ""

read -r -p "Start Vinaros now? (y/N): " REPLY
if [[ "\${REPLY:-}" =~ ^[Yy]$ ]]; then
  docker run -d --name vinaros -p "\${DOCKER_PORT}:8080" --restart unless-stopped "\${IMAGE_NAME}"
  echo ""
  echo "  Vinaros is running at http://localhost:\${DOCKER_PORT}"
fi
SETUP_EOF
...
# Vinaros Air-Gapped Deployment Bundle

**Vinaros v${APP_VERSION}** | Generated on $(date -u +"%Y-%m-%d %H:%M:%S UTC")
...
| \`vinaros-pymupdf-wasm-${PYMUPDF_VERSION}.tgz\` | PyMuPDF WASM module |
| \`vinaros-gs-wasm-${GS_VERSION}.tgz\` | Ghostscript WASM module |
...
3. Optionally start the Vinaros container
...
# PyMuPDF
tar xzf vinaros-pymupdf-wasm-${PYMUPDF_VERSION}.tgz -C ./wasm/pymupdf --strip-components=1

# Ghostscript (extract assets/ to root)
TEMP_GS=\$(mktemp -d)
tar xzf vinaros-gs-wasm-${GS_VERSION}.tgz -C \$TEMP_GS
...
\`\`\`bash
docker run -d --name vinaros -p 3000:8080 --restart unless-stopped ${IMAGE_NAME}
\`\`\`
