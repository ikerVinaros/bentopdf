...
**Vinaros** is a powerful, privacy-first, client-side PDF toolkit that is self hostable and allows you to manipulate, edit, merge, and process PDF files directly in your browser. No server-side processing is required, ensuring your files remain secure and private.

[![Docker Downloads](https://img.shields.io/badge/dynamic/json?url=https%3A%2F%2Fghcr-badge.elias.eu.org%2Fapi%2Falam00000%2Fvinaros%2Fvinaros&query=%24.downloadCount&logo=docker&label=Docker%20Downloads&color=blue)](https://github.com/alam00000/vinaros/pkgs/container/vinaros) [![Ko-fi](https://img.shields.io/badge/Buy%20me%20a%20Coffee-yellow?logo=kofi&style=flat-square)](https://ko-fi.com/alio01) ![GitHub Stars](https://img.shields.io/github/stars/alam00000/vinaros?style=social)
[![Sponsor me on GitHub](https://img.shields.io/badge/Sponsor-%E2%9D%A4-ff69b4)](https://github.com/sponsors/alam00000)

![Vinaros Tools](public/images/vinaros-tools.png)
...
[![Documentation](https://img.shields.io/badge/Docs-VitePress-646cff?style=for-the-badge&logo=vite&logoColor=white)](https://vinaros.com/docs/)

Visit our [Documentation](https://vinaros.com/docs/) for:
...
Vinaros is **dual-licensed** to fit your needs:
...
📖 For more details, see our [Licensing Page](https://vinaros.com/licensing.html)

### AGPL Components (Pre-configured via CDN)

Vinaros does **not** bundle AGPL-licensed processing libraries in its source code, but **pre-configures CDN URLs** so all features work out of the box with zero setup:
...
[![Star History Chart](https://api.star-history.com/svg?repos=alam00000/vinaros&type=Date)](https://star-history.com/#alam00000/vinaros&Date)

---

## 💖 Thank You to Our Sponsors

We're incredibly grateful to all our sponsors and supporters who help keep Vinaros free and open source!
...
- **High Performance**: Built with modern web technologies, Vinaros is fast and efficient, handling even large PDF files with ease.
- **Completely Free**: Vinaros is a free and open-source tool for everyone.

---

## 🛠️ Features / Tools Supported

Vinaros offers a comprehensive suite of tools to handle all your PDF needs.
...
Vinaros is available in multiple languages:
...
Want to help translate Vinaros into your language? Check out our [Translation Guide](TRANSLATION.md)!

---

## 🚀 Getting Started

You can run Vinaros locally for development or personal use.
...
Run Vinaros instantly from GitHub Container Registry (Recommended):

```bash
docker run -p 3000:8080 ghcr.io/alam00000/vinaros:latest
```
...
```bash
docker run -p 3000:8080 vinarosteam/vinaros:latest
```

**Podman (GHCR):**

```bash
podman run -p 3000:8080 ghcr.io/alam00000/vinaros:latest
```

**Podman (Docker Hub):**

```bash
podman run -p 3000:8080 docker.io/vinarosteam/vinaros:latest
```
...
It is very straightforward to host your own instance of Vinaros using a static web page hosting service. Plus, services such as Netlify, Vercel, and GitHub Pages all offer a free tier for getting started. See [Static Hosting](https://github.com/alam00000/vinaros/blob/main/STATIC-HOSTING.md) for details.

### 🏠 Self-Hosting Locally

Since Vinaros is fully client-side, all processing happens in the user's browser and no server-side processing is required. This means you can host Vinaros as simple static files on any web server or hosting platform.
...
The easiest way to self-host is to download the pre-built distribution file from our [GitHub releases](https://github.com/alam00000/vinaros/releases). Each release includes a `dist-{version}.zip` file that contains all necessary files for self-hosting.

1. Go to [Vinaros Releases](https://github.com/alam00000/vinaros/releases)
...
git clone https://github.com/alam00000/vinaros.git
cd vinaros
...
Vinaros supports different compression modes for optimized builds:

```bash
# Gzip only (smallest Docker image size)
npm run build:gzip
docker build --build-arg COMPRESSION_MODE=g -t vinaros:gzip .

# Brotli only (best compression ratio)
npm run build:brotli
docker build --build-arg COMPRESSION_MODE=b -t vinaros:brotli .

# No compression (fastest build time)
npm run build:original
docker build --build-arg COMPRESSION_MODE=o -t vinaros:original .

# All formats (default, maximum browser compatibility)
npm run build:all
docker build --build-arg COMPRESSION_MODE=all -t vinaros:all .
```
...
Vinaros can use jsDelivr CDN to serve large WASM files (LibreOffice, Ghostscript, PyMuPDF) for improved performance and reduced bandwidth costs:
...
```bash
VITE_WASM_PYMUPDF_URL=https://cdn.jsdelivr.net/npm/@vinaros/pymupdf-wasm@0.11.16/
VITE_WASM_GS_URL=https://cdn.jsdelivr.net/npm/@vinaros/gs-wasm/assets/
...
```
vinaros-airgap-bundle/
  vinaros.tar              # Docker image
...
| `--image-name <name>`          | Docker image tag                                 | `vinaros`                        |
| `--output-dir <path>`          | Output bundle directory                          | `./vinaros-airgap-bundle`        |
...
> [!IMPORTANT]
> WASM files must be served from the **same origin** as the Vinaros app. Web Workers use `importScripts()` which cannot load scripts cross-origin. For example, if Vinaros runs at `https://internal.example.com`, the WASM base URL should also be `https://internal.example.com/wasm`.
...
```bash
npm pack @vinaros/pymupdf-wasm@0.11.16
...
- `vinaros.tar` — the Docker image
- `vinaros-pymupdf-wasm-0.11.14.tgz` — PyMuPDF WASM package
- `vinaros-gs-wasm-*.tgz` — Ghostscript WASM package
...
tar xzf vinaros-pymupdf-wasm-0.11.14.tgz -C ./wasm/pymupdf --strip-components=1
tar xzf vinaros-gs-wasm-*.tgz -C ./wasm/gs --strip-components=1
...
Vinaros can also be hosted from a subdirectory (e.g., `example.com/tools/vinaros/`):

```bash

# Example:
# 1. Build the app with the specific BASE_URL. BASE_URL must have a trailing and leading slash. The BASE_URL can be any url of your choice. Here we are using /tools/vinaros/ as an example.

BASE_URL=/tools/vinaros/ npm run build
...
mkdir -p serve-test/tools/vinaros

# 3. Copy all files from the 'dist' folder into that nested directory
cp -r dist/* serve-test/tools/vinaros/

# 4. Serve the 'serve-test' folder
npx serve serve-test
```

The website can be accessible at: `http://localhost:3000/tools/vinaros/`
...
Vinaros's Docker image also supports the `BASE_URL` build argument for subdirectory deployments:

```bash
# Build for subdirectory deployment
docker build --build-arg BASE_URL=/vinaros/ -t vinaros .

# Run the container
docker run -p 3000:8080 vinaros

# The app will be accessible at http://localhost:3000/vinaros/
```
...
docker run -p 3000:8080 vinaros-simple
```

> [!IMPORTANT]
>
> - Always include trailing slashes in `BASE_URL` (e.g., `/vinaros/` not `/vinaros`)
...
services:
  vinaros:
    image: ghcr.io/alam00000/vinaros:latest # Recommended
    # image: vinarosteam/vinaros:latest     # Alternative: Docker Hub
...
For Linux production deployments, you can run Vinaros as a systemd service using [Podman Quadlet](https://docs.podman.io/en/latest/markdown/podman-systemd.unit.5.html).

Create `~/.config/containers/systemd/vinaros.container`:
...
systemctl --user enable --now vinaros
```

For detailed Quadlet configuration, see [Self-Hosting Docker Guide](https://vinaros.com/docs/self-hosting/docker).

### 🏢 Simple Mode for Internal Use

For organizations that want a clean, distraction-free interface focused solely on PDF tools, Vinaros supports a **Simple Mode** that hides all branding and marketing content.
...
Replace the default Vinaros logo, name, and footer text with your own. Branding is configured via environment variables at **build time** and works across all deployment methods (Docker, static hosting, air-gapped VMs).

| Variable           | Description                             | Default                                 |
| ------------------ | --------------------------------------- | --------------------------------------- |
| `VITE_BRAND_NAME`  | Brand name shown in header and footer   | `Vinaros`                              |
...
Vinaros runs as a non-root user using nginx-unprivileged for enhanced security:
...
```bash
docker build -t vinaros .
docker run -p 8080:8080 vinaros
```
...
docker run -d -p 3000:8080 -e PUID=1000 -e PGID=1000 vinaros-nonroot
```
...
3. **Update allowed origins** — open `cors-proxy-worker.js` and change `ALLOWED_ORIGINS` to your domain:
   ...
   > [!IMPORTANT]
   > Without this step, the proxy will reject all requests from your site with a 403 error. The default only allows `vinaros.com`.
...
5. **Note your worker URL** (e.g., `https://vinaros-cors-proxy.your-subdomain.workers.dev`)
...
Vinaros uses client-side HMAC as a deterrent against casual abuse, but accepts this tradeoff due to its fully client-side architecture. To enable:
...
Vinaros supports semantic versioning with multiple container tags available:

**GitHub Container Registry (Recommended):**

- **Latest**: `ghcr.io/alam00000/vinaros:latest`
- **Specific Version**: `ghcr.io/alam00000/vinaros:1.0.0`
- **Version with Prefix**: `ghcr.io/alam00000/vinaros:v1.0.0`

**Docker Hub:**

- **Latest**: `vinarosteam/vinaros:latest`
- **Specific Version**: `vinarosteam/vinaros:1.0.0`
- **Version with Prefix**: `vinarosteam/vinaros:v1.0.0`
...
Vinaros was originally built using **HTML**, **CSS**, and **vanilla JavaScript**. As the project grew, it was migrated to a modern stack for better maintainability and scalability:
...
Have an idea for a new tool or an improvement? [Open an issue](https://github.com/alam00000/vinaros/issues) to discuss it first.
...
Vinaros wouldn't be possible without the amazing open-source tools and libraries that power it. We'd like to extend our heartfelt thanks to the creators and maintainers of:
...
> [!NOTE]
> AGPL-licensed libraries are not bundled in Vinaros's source code. They are loaded at runtime from CDN (pre-configured) and can be overridden via environment variables or Advanced Settings.
