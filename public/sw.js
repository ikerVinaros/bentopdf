...
const CACHE_VERSION = 'vinaros-v10';
      ...
      .then((cacheNames) => {
        return Promise.all(
          cacheNames.map((cacheName) => {
            if (cacheName.startsWith('vinaros-') && cacheName !== CACHE_NAME) {
...
function shouldCache(pathname, isCDN = false) {
  if (isCDN) {
    return (
      pathname.includes('/@vinaros/pymupdf-wasm') ||
      pathname.includes('/@vinaros/gs-wasm') ||
