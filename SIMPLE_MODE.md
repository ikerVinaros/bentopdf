...
```bash
# Docker
docker run -p 3000:8080 ghcr.io/alam00000/vinaros-simple:latest

# Podman
podman run -p 3000:8080 ghcr.io/alam00000/vinaros-simple:latest
```
...
# Podman
podman run -p 3000:8080 docker.io/vinarosteam/vinaros-simple:latest
```
...
services:
  vinaros:
    ...
    # Or using Docker Hub
    # image: vinarosteam/vinaros-simple:latest
...
```bash
docker build --build-arg SIMPLE_MODE=true -t vinaros-simple .
...
docker pull ghcr.io/alam00000/vinaros-simple:latest
docker run -p 3000:8080 ghcr.io/alam00000/vinaros-simple:latest

# Podman
podman pull ghcr.io/alam00000/vinaros-simple:latest
podman run -p 3000:8080 ghcr.io/alam00000/vinaros-simple:latest
```
...
docker run -p 3001:8080 ghcr.io/alam00000/vinaros-simple:latest
...
- `ghcr.io/alam00000/vinaros:v1.0.0` (versioned)

**Docker Hub:**

- `vinarosteam/vinaros:latest`
- `vinarosteam/vinaros:v1.0.0` (versioned)

### Simple Mode (Clean Interface)

**GitHub Container Registry (Recommended):**

- `ghcr.io/alam00000/vinaros-simple:latest`
- `ghcr.io/alam00000/vinaros-simple:v1.0.0` (versioned)

**Docker Hub:**

- `vinarosteam/vinaros-simple:latest`
- `vinarosteam/vinaros-simple:v1.0.0` (versioned)
...
services:
  vinaros:
    image: ghcr.io/alam00000/vinaros-simple:latest # Recommended
    # image: vinarosteam/vinaros-simple:latest     # Alternative: Docker Hub
    container_name: vinaros
    restart: unless-stopped
    ports:
      - '80:8080'
...
Create `~/.config/containers/systemd/vinaros-simple.container`:

```ini
[Unit]
Description=Vinaros Simple Mode
...
systemctl --user enable --now vinaros-simple
```

## ⚠️ Important Notes

- **Pre-built images**: Use `ghcr.io/alam00000/vinaros-simple:latest` for Simple Mode (recommended)
