...
Deploys **Vinaros** as a **single NGINX container** serving the static frontend.
...
```bash
helm install vinaros ./chart
kubectl port-forward deploy/vinaros 8080:8080
...
- **`image.repository`**: container image repo (default: `ghcr.io/alam00000/vinaros-simple`)
- **`image.tag`**: image tag (default: `Chart.appVersion`)
- **`image.pullPolicy`**: default `IfNotPresent`

### Ports

- **`containerPort`**: container listen port (**8080** for the Vinaros nginx image)
...
# produces vinaros-<version>.tgz
helm push vinaros-*.tgz oci://ghcr.io/$GHCR_USERNAME/charts
```
...
```bash
helm upgrade --install vinaros oci://ghcr.io/$GHCR_USERNAME/charts/vinaros --version 1.0.0
```
