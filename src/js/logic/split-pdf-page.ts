...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const split = async () => {
    ...
    try {
      ...
      switch (splitMode) {
        case 'range':
          ...
          if (rangeGroups.length > 1) {
            ...
            showAlert(
              t('alert.success'),
              t('tools:splitPdf.successMultiple', { count: rangeGroups.length }),
              'success',
              () => {
                resetState();
              }
        ...
        case 'bookmarks':
          ...
          hideLoader();
          showAlert(t('alert.success'), t('tools:splitPdf.successSingle'), 'success', () => {
            resetState();
          });
          return;
      ...
      showAlert(
        t('alert.error'),
        e.message || t('tools:splitPdf.errorSplit')
      );
    } finally {
      hideLoader();
    }
