...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const compress = async () => {
    ...
    try {
      if (state.files.length === 0) {
        showAlert(t('tools:compressPdf.noFiles'), t('tools:compressPdf.selectAtLeastOne'));
        hideLoader();
        return;
      }
      ...
      if (state.files.length === 1) {
        ...
        if (savings > 0) {
          showAlert(
            t('tools:compressPdf.compressionComplete'),
          ...
          showAlert(
            t('tools:compressPdf.compressionFinished'),
            t('tools:compressPdf.noSavings'),
            'warning',
            () => resetState()
          );
        }
      ...
      showAlert(
        t('alert.error'),
        t('tools:compressPdf.errorCompress') + ` Error: ${e.message}`
      );
    }
  };
