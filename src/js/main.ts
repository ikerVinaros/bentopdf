...
import { dom, switchView, hideAlert, showAlert } from './ui.js';
...
// Custom Popular Tools Storage Key
const POPULAR_TOOLS_STORAGE_KEY = 'customPopularTools';
...
const init = async () => {
  ...
  if (__SIMPLE_MODE__) {
    const hideBrandingSections = () => {
      ...
      const githubLink = document.querySelector(
        'a[href*="github.com/alam00000/vinaros"]'
      );
      if (githubLink) {
      ...
      const brandName = __BRAND_NAME__ || 'Vinaros';
  ...
  if (dom.toolGrid) {
    ...
    function saveCollapsedCategories() {
      ...
      // Function to customize Popular Tools
    function openPopularToolsCustomizer() {
      // Get all tools and REMOVE DUPLICATES by name
      const allToolsMap = new Map();
      categories.flatMap((c) => c.tools).forEach(tool => {
      ...
      const stored = localStorage.getItem(POPULAR_TOOLS_STORAGE_KEY);
      
      if (stored && stored !== 'null' && stored !== 'undefined' && stored !== '[]') {
        try {
          selectedTools = JSON.parse(stored);
        } catch {
          selectedTools = [];
        }
      }
      
      // Only use defaults if nothing is saved
      if (selectedTools.length === 0) {
        const popularCategory = categories.find((c) => c.name === 'Popular Tools');
        if (popularCategory) {
          selectedTools = popularCategory.tools.map((t) => t.name);
        }
      }

      // Build tool selection HTML
      // Complete mapping from tool name (English) to translation key
      const toolNameToKey: Record<string, string> = {
        ...
        'OCR PDF': 'ocrPdf',
        'Alternate & Mix Pages': 'alternateMix',
        'Add Attachments': 'addAttachments',
        'Extract Attachments': 'extractAttachments',
        'Edit Attachments': 'editAttachments',
        'Divide Pages': 'dividePages',
        'Add Blank Page': 'addBlankPage',
        'Reverse Pages': 'reversePages',
        'Rotate PDF': 'rotatePdf',
        'Rotate by Custom Degrees': 'rotateCustom',
        'N-Up PDF': 'nUpPdf',
        'Combine to Single Page': 'combineToSinglePage',
        'View Metadata': 'viewMetadata',
        'Edit Metadata': 'editMetadata',
        'PDFs to ZIP': 'pdfsToZip',
        'Compare PDFs': 'comparePdfs',
        'Posterize PDF': 'posterizePdf',
        'Fix Page Size': 'fixPageSize',
        ...
        'PSD to PDF': 'psdToPdf',
        'PDF to SVG': 'pdfToSvg',
        'Extract PDF Tables': 'extractTables',
        'PDF to CSV': 'pdfToCsv',
        'PDF to Excel': 'pdfToExcel',
        'PDF to Text': 'pdfToText',
        'Digital Signature PDF': 'digitalSignPdf',
        'Validate PDF Signature': 'validateSignaturePdf',
        'Email to PDF': 'emailToPdf',
        'Font to Outline': 'fontToOutline',
        'Deskew PDF': 'deskewPdf',
        'PDF to Word': 'pdfToWord',
        'Extract Images': 'extractImages',
        'PDF to Markdown': 'pdfToMarkdown',
        'Prepare PDF for AI': 'preparePdfForAi',
        'PDF OCG': 'pdfOcg',
        'PDF to PDF/A': 'pdfToPdfa',
        'Rasterize PDF': 'rasterizePdf',
        'PDF Workflow Builder': 'pdfWorkflow',
        // Additional tools from the file
        'Organize & Duplicate': 'duplicateOrganize',
        'Extract Tables': 'extractTables',
        'Digital Signature': 'digitalSignPdf',
        'Validate Signature': 'validateSignaturePdf',
      };

      const toolOptions = allTools.map((tool) => {
        const isSelected = selectedTools.includes(tool.name);
        // Get the translation key from the tool name
        const toolKey = toolNameToKey[tool.name] || tool.name;
        const toolTranslationKey = `tools:${toolKey}.name`;
        const translatedName = t(toolTranslationKey) || tool.name;
        return `
          <label class="flex items-center gap-2 p-2 hover:bg-gray-100 rounded cursor-pointer">
            <input type="checkbox" value="${tool.name}" ${isSelected ? 'checked' : ''} class="w-4 h-4 text-indigo-600 rounded">
      ...
      const modalHtml = `
        <div id="popular-tools-modal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div class="bg-white rounded-lg shadow-xl max-w-md w-full max-h-[80vh] overflow-hidden">
            <div class="p-4 border-b flex justify-between items-center">
              <h3 class="text-lg font-bold">${t('tools:customizePopularTools') || 'Customize Popular Tools'}</h3>
            ...
            <div class="p-4 overflow-y-auto max-h-[60vh]">
              <p class="text-sm text-gray-600 mb-4">${t('tools:selectToolsToShow') || 'Select the tools you want to show in Popular Tools:'}</p>
              <div class="grid grid-cols-1 gap-1" id="tool-options">
                ${toolOptions}
              </div>
            </div>
            <div class="p-4 border-t flex justify-between gap-2">
              <button id="reset-popular-tools" class="px-4 py-2 text-sm text-gray-600 hover:text-gray-800">${t('tools:resetToDefault') || 'Reset to Default'}</button>
              <div class="flex gap-2">
                <button id="cancel-popular-tools" class="px-4 py-2 text-sm text-gray-600 hover:bg-gray-100 rounded">${t('common.cancel') || 'Cancel'}</button>
                <button id="save-popular-tools" class="px-4 py-2 text-sm bg-indigo-600 text-white hover:bg-indigo-700 rounded">${t('common.save') || 'Save'}</button>
              </div>
            </div>
          </div>
        </div>
      ...
      const existingModal = document.getElementById('popular-tools-modal');
      ...
      document.getElementById('close-modal')?.addEventListener('click', () => {
        document.getElementById('popular-tools-modal')?.remove();
      });

      document.getElementById('cancel-popular-tools')?.addEventListener('click', () => {
        document.getElementById('popular-tools-modal')?.remove();
      });

      document.getElementById('save-popular-tools')?.addEventListener('click', () => {
        // Get FRESH checkbox states directly from DOM - don't use any pre-loaded values
        const checkboxes = document.querySelectorAll('#tool-options input[type="checkbox"]');
        ...
        // Save to localStorage - ensure it's a string
        const storageValue = JSON.stringify(uniqueSelected);
        localStorage.setItem(POPULAR_TOOLS_STORAGE_KEY, storageValue);
        
        // Verify the save IMMEDIATELY
        const verify = localStorage.getItem(POPULAR_TOOLS_STORAGE_KEY);
      ...
      document.getElementById('reset-popular-tools')?.addEventListener('click', () => {
        localStorage.removeItem(POPULAR_TOOLS_STORAGE_KEY);
        document.getElementById('popular-tools-modal')?.remove();
        window.location.reload();
      });

      // Close on background click
      document.getElementById('popular-tools-modal')?.addEventListener('click', (e) => {
    ...
    // Get custom popular tools from localStorage
    function getCustomPopularTools(): string[] | null {
      try {
        const stored = localStorage.getItem(POPULAR_TOOLS_STORAGE_KEY);
        if (stored && stored !== 'null' && stored !== 'undefined' && stored !== '[]') {
        ...
        console.error('Error reading popular tools:', e);
      }
      return null;
    }
    ...
    categories.forEach((category) => {
      ...
      chevron.className =
        'category-chevron w-5 h-5 text-gray-500 transition-transform duration-300';

      header.append(title, chevron);

      // Add customize button for Popular Tools
      if (category.name === 'Popular Tools') {
        const customizeBtn = document.createElement('button');
        customizeBtn.type = 'button';
        customizeBtn.className = 'ml-2 p-1 text-gray-400 hover:text-indigo-600 hover:bg-gray-100 rounded';
        customizeBtn.title = t('tools:customizePopularTools') || 'Customize';
        customizeBtn.innerHTML = '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path></svg>';
        customizeBtn.addEventListener('click', (e) => {
          e.stopPropagation();
          openPopularToolsCustomizer();
        });
      ...
      // Apply custom popular tools selection
      let toolsToRender = category.tools;
      if (category.name === 'Popular Tools') {
        const customTools = getCustomPopularTools();
        if (customTools && customTools.length > 0) {
          toolsToRender = category.tools.filter((tool) => {
            return customTools.includes(tool.name);
          });
        }
      }
  ...
  if (githubStarsElements.some((el) => el) && !__SIMPLE_MODE__) {
    fetch('https://api.github.com/repos/alam00000/vinaros')
