...
echo "Starting Vinaros with PUID=$PUID PGID=$PGID"

addgroup -g "$PGID" vinaros 2>/dev/null || true
adduser -u "$PUID" -G vinaros -D -H -s /sbin/nologin vinaros 2>/dev/null || true
