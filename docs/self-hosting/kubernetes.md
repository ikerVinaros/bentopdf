...
> 
> The official Vinaros nginx images include these headers. In Kubernetes, **Ingress/Gateway controllers are also reverse proxies**, so ensure these headers are preserved (or add them at the edge).

## Prereqs

- Kubernetes cluster
- Helm v3
- A Vinaros nginx image (e.g. `ghcr.io/alam00000/vinaros:<tag>`) that serves on **port 8080**
...
```bash
kubectl create namespace vinaros

helm upgrade --install vinaros /path/to/vinaros/chart \
  --namespace vinaros \
  --set image.repository=ghcr.io/alam00000/vinaros \
...
helm upgrade --install vinaros oci://ghcr.io/$GHCR_USERNAME/charts/vinaros \
  --namespace vinaros \
  --create-namespace \
  --version 0.1.0 \
  --set image.repository=ghcr.io/alam00000/vinaros \
...
```bash
kubectl -n vinaros port-forward deploy/vinaros 8080:8080
```
...
Vinaros’s nginx config sets the required response headers. Most Ingress/Gateway controllers **pass upstream response headers through unchanged**.
