...
[Hostinger](https://hostinger.com) is a popular shared hosting provider. This guide covers deploying Vinaros to Hostinger's shared hosting.
...
RewriteCond %{HTTP_HOST} ^vinaros\.com [NC]
RewriteRule ^(.*)$ https://www.vinaros.com/$1 [L,R=301]
...
Make sure the `.html` extension rule and language routes are correctly configured. Vinaros uses static HTML files, not SPA routing:
