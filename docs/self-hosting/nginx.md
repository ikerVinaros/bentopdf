...
Host Vinaros on your own server using Nginx.
...
Create `/etc/nginx/sites-available/vinaros`:
...
```bash
sudo ln -s /etc/nginx/sites-available/vinaros /etc/nginx/sites-enabled/
sudo nginx -t
