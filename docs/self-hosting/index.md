...
Vinaros can be self-hosted on your own infrastructure. This guide covers various deployment options.

## Quick Start with Docker / Podman

The fastest way to self-host Vinaros:
...
>
> Use `http://localhost` only for same-device testing. If users access Vinaros through a LAN IP or hostname, terminate it with HTTPS.

```bash
# Docker
docker run -d -p 3000:8080 ghcr.io/alam00000/vinaros:latest

# Podman
podman run -d -p 3000:8080 ghcr.io/alam00000/vinaros:latest
```
...
Run Vinaros as a systemd service. Create `~/.config/containers/systemd/vinaros.container`:
...
docker run -p 3000:8080 vinarosteam/vinaros-simple:latest
```

See [SIMPLE_MODE.md](https://github.com/alam00000/vinaros/blob/main/SIMPLE_MODE.md) for full details.
...
Replace the default Vinaros logo, name, and footer text with your own at build time:
...
Vinaros **does not bundle** AGPL-licensed processing libraries in its source code, but **pre-configures CDN URLs** so all features work out of the box — no manual setup needed.
...
If you need to serve AGPL WASM files with proper CORS headers, you can deploy a simple proxy. See the [Cloudflare WASM Proxy guide](https://github.com/alam00000/vinaros/blob/main/cloudflare/WASM-PROXY.md) for an example implementation.

::: tip Why Separate?
This separation ensures:

- Clear legal compliance for commercial users
- Vinaros's core remains under its dual-license (AGPL-3.0 / Commercial)
...
::: tip
Vinaros is a static site—there's no database or backend server required!
:::
